...
// then caps at max. The mainworld's own world.RollPopulation has a reroll-on-10
...
func dmPopulation(r *dice.Roller, dm int, maxPopulation ehex.Value) ehex.Value {
	return capPopulation(world.ClampEhex(r.TwoD6()-2+dm, 0, int(ehex.Max)), maxPopulation)
}

// secondaryPopulation rolls Population for a secondary world: the
// standard world.RollPopulation (including its reroll-on-10 exception) when
...
func secondaryPopulation(r *dice.Roller, dm int, maxPopulation ehex.Value) ehex.Value {
	if dm == 0 {
		return capPopulation(world.RollPopulation(r), maxPopulation)
	}

	return dmPopulation(r, dm, maxPopulation)
}

// dmAtmosphere rolls Atmosphere via the standard world.RollAtmosphere formula
// plus an additional dm, clamped to 0..15 (F). dm=0 reproduces
// world.RollAtmosphere's own result exactly.
func dmAtmosphere(r *dice.Roller, size ehex.Value, dm int) ehex.Value {
	return world.ClampEhex(int(world.RollAtmosphere(r, size))+dm, 0, 15)
}

// dmHydrographics rolls Hydrographics via the standard world.RollHydrographics
// formula plus an additional dm, clamped to 0..10 (A). dm=0 reproduces
// world.RollHydrographics's own result exactly.
func dmHydrographics(r *dice.Roller, size, atm ehex.Value, dm int) ehex.Value {
	return world.ClampEhex(int(world.RollHydrographics(r, size, atm))+dm, 0, 10)
}
...
func rollSecondaryUWPBody(
	...
	u.Starport = world.RollStarport(r)
	u.Size = sizeRoll(r)
	...
	u.Government = world.RollGovernment(r, u.Population)
	u.Law = world.RollLaw(r, u.Government)
	u.TechLevel = world.RollTechLevel(r, u)
...
func generateHospitableWorld(r *dice.Roller, maxPopulation ehex.Value) world.UWP {
	return rollSecondaryUWPBody(r, world.RollSize, 0, 0, 0, maxPopulation)
}
...
func generateBigWorld(r *dice.Roller, maxPopulation ehex.Value) world.UWP {
...
func generateRadWorld(r *dice.Roller) world.UWP {
	var u world.UWP

	u.Starport = world.RollStarport(r)
	u.Size = world.ClampEhex(r.TwoD6(), 0, int(ehex.Max))
	u.Atmosphere = world.RollAtmosphere(r, u.Size)
	u.Hydrographics = world.RollHydrographics(r, u.Size, u.Atmosphere)
...
// this project's usual world.StarportNone representation. Read here as
// Starport=world.StarportNone (T5's "no starport" case) and every other field
// fixed at 0 — a defensible, simple reading, not a claim of certainty.
func generateInferno(r *dice.Roller) world.UWP {
	return world.UWP{
		Starport: world.StarportNone,
		Size:     world.ClampEhex(6+r.D6(), 0, int(ehex.Max)),
	}
}

// generateWorldlet: Book 3 p.29's Worldlet= StSAHPGL-T Siz=1D-3 (floored
// at 0), rest normal.
func generateWorldlet(r *dice.Roller, maxPopulation ehex.Value) world.UWP {
...
func generateIceworld(r *dice.Roller, maxPopulation ehex.Value) world.UWP {
	return rollSecondaryUWPBody(r, world.RollSize, 0, 0, -6, maxPopulation)
}

// generateInnerWorld: Book 3 p.29's Inner World= StSAHPGL-T Pop=DM-4
// Hyd=DM-4.
func generateInnerWorld(r *dice.Roller, maxPopulation ehex.Value) world.UWP {
	return rollSecondaryUWPBody(r, world.RollSize, 0, -4, -4, maxPopulation)
}

// generateStormWorld: Book 3 p.29's Stormworld= StSAHPGL-T Siz=2D
// Atm=DM+4 Hyd=DM-4 Pop=DM-6.
func generateStormWorld(r *dice.Roller, maxPopulation ehex.Value) world.UWP {
...
func generatePlanetoidWorld(r *dice.Roller, maxPopulation ehex.Value) world.UWP {
	var u world.UWP

	u.Starport = world.RollStarport(r)
	u.Population = capPopulation(world.RollPopulation(r), maxPopulation)
	u.Government = world.RollGovernment(r, u.Population)
	u.Law = world.RollLaw(r, u.Government)
	u.TechLevel = world.RollTechLevel(r, u)
...
func generateSecondaryWorldUWP(r *dice.Roller, category secondaryWorldCategory, maxPopulation ehex.Value) world.UWP {
	switch category {
