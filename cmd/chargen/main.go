...
// Markdown. Eight careers exist so far — Scout, Citizen, Noble, Marine,
// Soldier, Spacer, Rogue, and Scholar, selected via -career — see
...
// character/rogue_character_generate.go, and
// character/scholar_character_generate.go for what's generated and
...
// Major/Minor selection and Waivers, Craftsman/Functionary (both
// architecturally blocked, see the chargen plan history), and the other
// 4 T5 careers).
package main
...
func main() {
	careerName := flag.String(
		"career",
		"scout",
		"career to generate: scout, citizen, noble, marine, soldier, spacer, rogue, or scholar",
	)
	...
	if !ok {
		...
		fmt.Fprintln(
			...
			"chargen: this attempt did not survive character generation, or never qualified "+
				...
				"Scholar: Begin failed or died, p.76)",
		)
		os.Exit(1)
	}
}
