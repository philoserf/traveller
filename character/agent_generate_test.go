...
//
// Only the outcome each test is named for is pinned. Which Undercover
// Assignment the seed happens to draw is deliberately *not* asserted:
// it is incidental to Risk and Reward, and pinning it made these tests
// fail for the wrong reason every time the dice stream shifted. What is
// asserted instead is the invariant that actually matters here — the
// Commendation label names the career that was actually assigned.

// assertAgentCommendationNamesAssignment checks Book 1 p.83's own
// "<Undercover Career> Commendation-N" labelling against the term's own
// assignment, and that the assignment is one the p.83 table can produce.
func assertAgentCommendationNamesAssignment(t *testing.T, term Term) {
	t.Helper()

	if _, ok := agentUndercoverSkillTables[term.UndercoverCareer]; !ok && term.UndercoverCareer != CitizenCareerName {
		t.Errorf("UndercoverCareer = %q, which the p.83 table cannot produce", term.UndercoverCareer)
	}

	if term.UndercoverAssignment == "" {
		t.Errorf("UndercoverAssignment is empty — every p.83 row prints at least a service label")
	}
	...
	if !strings.HasPrefix(term.RewardResult, term.UndercoverCareer+" Commendation-") {
		t.Errorf("RewardResult = %q, want it to name the assigned career %q",
			term.RewardResult, term.UndercoverCareer)
	}
}
...
func TestResolveAgentTermRiskSuccessRewardSuccess(t *testing.T) {
	...
	term, _ := ResolveAgentTerm(dice.New(rand.NewPCG(2, 2)), uppAgent88, C1)
	...
	if term.RewardResult == "None" {
		t.Fatalf("RewardResult = %q, want a Commendation", term.RewardResult)
	}

	assertAgentCommendationNamesAssignment(t, term)
}

func TestResolveAgentTermRiskSuccessRewardFailure(t *testing.T) {
	t.Parallel()

	term, _ := ResolveAgentTerm(dice.New(rand.NewPCG(5, 5)), uppAgent88, C1)
	...
	if term.RewardResult != "None" {
		t.Errorf("RewardResult = %q, want \"None\"", term.RewardResult)
	}
...
func TestResolveAgentTermRiskFailureReducesCharacteristic(t *testing.T) {
	...
	term, updatedUPP := ResolveAgentTerm(dice.New(rand.NewPCG(1, 1)), uppAgent88, C1)
